...
static float path_heuristic(void *fromNode, void *toNode, void *context)
{
	...
	float absX = (float)fabs((double)(from->x - to->x));
	float absY = (float)fabs((double)(from->y - to->y));
...
static int lua__path(lua_State *L)
{
	...
	if (from.x < 0 || from.x > map->w || from.y < 0 || from.y > map->h) {
		lua_pushboolean(L, 0);
		lua_pushfstring(L, "from point(%d, %d), map(%d, %d) error",
				from.x, from.y,
				map->w, map->h);
		return 2;
	}
	if (to.x < 0 || to.x > map->w || to.y < 0 || to.y > map->h) {
		lua_pushboolean(L, 0);
		lua_pushfstring(L, "to point(%d, %d), map(%d, %d) error",
				to.x, to.y,
				map->w, map->h);
		return 2;
	}
...
static int opencls__astar_map(lua_State *L)
{
	luaL_Reg lmethods[] = {
		{"path", lua__path},
		{NULL, NULL},
