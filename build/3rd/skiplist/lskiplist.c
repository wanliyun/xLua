...
static int lua__sl_new(lua_State *L)
{
	sl_t *sl;
...
static int lua__sl_skiplist_gc(lua_State *L)
{
...
static int lua__sl_insert(lua_State *L)
{
	double score;
	sl_t *sl;
	slNode_t *node;
	...
	if ((node = slCreateNode(level, NULL, score)) == NULL)
		return luaL_error(L, "no memory in lua__sl_insert");
...
static int lua__sl_update(lua_State *L)
{
	int level;
	double score;
	int cur = 0;
	...
	if (node != NULL) {
		...
		level = slRandomLevel();
		if ((node = slCreateNode(level, NULL, score)) == NULL) {
			return luaL_error(L, "no memory in lua__sl_update");
		}
		node->udata = node;
		lua_getuservalue(L, 1);
			lua_getfield(L, -1, "node_map");
...
#if ENABLE_LSL_INDEX
static int lua__sl_index(lua_State *L)
{
	slNode_t *node;
	sl_t *sl = CHECK_SL(L, 1);
	(void)sl;
...
static int lua__sl_delete(lua_State *L)
{
	int ret;
	int cur = 0;
	sl_t *sl = CHECK_SL(L, 1);
...
static int lua__sl_exists(lua_State *L)
{
	sl_t *sl = CHECK_SL(L, 1);
	slNode_t *node = luac__get_node(L, 1, 2);
	(void)sl;
...
static int lua__sl_size(lua_State *L)
{
	sl_t *sl = CHECK_SL(L, 1);
...
static int lua__sl_rank_iterator(lua_State *L)
{
	slNode_t *node;
	slNode_t *next;
...
static int lua__sl_rank_pairs(lua_State *L)
{
	slNode_t *node;
	sl_t *sl = CHECK_SL(L, 1);
	...
	lua_pushlightuserdata(L, (void *)node);
	lua_pushcclosure(L, lua__sl_rank_iterator, 4);
	return 1;
}

static int lua__sl_get_by_rank(lua_State *L)
{
	slNode_t *node;
	sl_t *sl = CHECK_SL(L, 1);
...
static int lua__sl_del_by_rank(lua_State *L)
{
	int cur = 0;
	int ret;
	slNode_t *node;
...
static int lua__sl_get_score(lua_State *L)
{
	sl_t *sl = CHECK_SL(L, 1);
	slNode_t *node = luac__get_node(L, 1, 2);
	(void)sl;
...
static int lua__sl_score_range(lua_State *L)
{
	int i;
...
static int lua__sl_rank_of(lua_State *L)
{
	int rank = 0;
	sl_t *sl = CHECK_SL(L, 1);
	int cur = 0;
...
static int lua__sl_rank_range(lua_State *L)
{
	int n;
	sl_t *sl = CHECK_SL(L, 1);
	slNode_t *node;
...
static int lua__sl_next(lua_State *L)
{
	slNode_t *next = NULL;
	sl_t *sl = CHECK_SL(L, 1);
	slNode_t *node = luac__get_node(L, 1, 2);
...
static int lua__sl_prev(lua_State *L)
{
	slNode_t *prev = NULL;
	sl_t *sl = CHECK_SL(L, 1);
	slNode_t *node = luac__get_node(L, 1, 2);
...
static void deleteCb(void *udata, void *ctx)
{
	lua_State *L = ctx;
	int top = lua_gettop(L);
	/* node->udata = node, see lua__sl_insert */ 
...
static int lua__sl_del_rank_range(lua_State *L)
{
	sl_t *sl = CHECK_SL(L, 1);
...
{
	luaL_Reg lmethods[] = {
		{"insert", lua__sl_insert},
		{"update", lua__sl_update},
		{"delete", lua__sl_delete},
		{"exists", lua__sl_exists},
		{"get_by_rank", lua__sl_get_by_rank},
		{"del_by_rank", lua__sl_del_by_rank},
		{"del_by_rank_range", lua__sl_del_rank_range},
		{"rank_of", lua__sl_rank_of},
		{"rank_range", lua__sl_rank_range},
		{"get_score", lua__sl_get_score},
		{"score_range", lua__sl_score_range},
		{"next", lua__sl_next},
		{"prev", lua__sl_prev},
		{"size", lua__sl_size},
		{"rank_pairs", lua__sl_rank_pairs},
		{NULL, NULL},
	};
	luaL_newmetatable(L, CLASS_SKIPLIST);
	luaL_newlib(L, lmethods);
#if ENABLE_LSL_INDEX
	lua_pushcclosure(L, lua__sl_index, 1);
#endif
	lua_setfield(L, -2, "__index");
	lua_pushcfunction(L, lua__sl_update);
	lua_setfield(L, -2, "__newindex");
	lua_pushcfunction(L, lua__sl_skiplist_gc);
	lua_setfield (L, -2, "__gc");
	lua_pushcfunction(L, lua__sl_size);
