...
#define XXTEA_ALIGN(n, a) (((n) & (a-1)) == 0) ? (n) : ((n) + a - ((n) & (a-1)))

#define DEFAULT_BUFF_ARRAY_SIZE 32 * 1024 /*32 KB*/
...
static void getxxt_key(const unsigned char *key, xxtea_length_t key_length, uint32_t *k)
{
	unsigned char buf[5];
			     ...
			     xxtea_length_t *dstlen)
{
	...
	getxxt_key(key, keylen, k);

	l = XXTEA_ALIGN(srclen + sizeof(srclen), 4);
...
/*	
	...
	compile in gcc with
	gcc --shared -DENABLE_LUA_XXTEA -fPIC -O2 -o xxtea.so xxtea.c -L/usr/local/lib -llua
	use in lua
	require'xxtea'
	str = 'something'
...
static int lua__loadstring(lua_State *L)
{
	size_t prefix_size;
	size_t key_length;
	size_t en_length;
	const char *prefix = luaL_checklstring(L, 1, &prefix_size);
	const char *key = luaL_checklstring(L, 2, &key_length);
	const char *encrypt_data = luaL_checklstring(L, 3, &en_length);
	const char *name = luaL_optstring(L, 4, "<unkown>");
	const char *source = encrypt_data;
	size_t source_size = en_length;
	unsigned char * decrypt_data = NULL;
	int ret;

	if (prefix_size > 0 && strncmp(prefix, encrypt_data, prefix_size) == 0) {
		xxtea_length_t ret_length;
		unsigned char *decrypt_data = xxtea_decrypt((const unsigned char *)encrypt_data + prefix_size,
							    (xxtea_length_t)en_length - prefix_size,
							    (const unsigned char *)key,
							    (xxtea_length_t)key_length,
							    &ret_length);
		source = (const char *)xxtea_getsource(decrypt_data, ret_length);
		source_size = ret_length;
	}
	ret = luaL_loadbuffer(L, source, source_size, name);
